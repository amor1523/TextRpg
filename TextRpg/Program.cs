namespace TextRpg
{
    ...
    //과제 피드백
    //기능에 따라 클래스를 구분한 점이 좋고 코드 구성이 전반적으로 깔끔하고 가독성도 높습니다.
    //다만, 파일을 나눠서 관리해주세요.
    //기능에 적합한 자료구조를 선택하고 활용한 점도 좋습니다.

    //기본적인 문자열 포맷팅, 문법 핸들링에 큰 어려움을 보이지 않고 있는 거 같습니다.

    //인벤토리나 상점도 별도 클래스로 관리해 필요한 기능을 재구성해보세요.

    //아이템 장착/해제/구매/판매와 관련해선 반환값이 있는 메소드를 통해 데이터 입출력을 구현해보세요.

    //장착 개선의 경우, 플레이어의 장비창을 만든다고 생각하면 좋을 것 같습니다.
    //혹은 인벤토리 내에서 타입에 따라 장착된 아이템인지 조회하는 bool 값 반환 메소드를 작성하는 방법도 있습니다.

    //씬 관리도 개별 클래스로 관리해보세요.
